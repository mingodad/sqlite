...
static void broundFunc(sqlite3_context *context, int argc, sqlite3_value **argv){
    sqlite_double num, dec_factor, tmp, itmp;
    int neg, udec_places = 0;
	switch(argc){
	  case 2:
	    if( SQLITE_NULL==sqlite3_value_type(argv[1]) ) return;
		udec_places = sqlite3_value_int(argv[1]);
		if( udec_places>30 ) udec_places = 30;
		else if( udec_places<0 ) udec_places = 0;
	  case 1:
		if( SQLITE_NULL==sqlite3_value_type(argv[0]) ) return;
		num = sqlite3_value_double(argv[0]);
		break;
	  default:
		return;
	}
    ...
    switch(udec_places){
        case 0: dec_factor = LITDBL(1.0); break;
        case 1: dec_factor = LITDBL(10.0); break;
        case 2: dec_factor = LITDBL(100.0); break;
        case 3: dec_factor = LITDBL(1000.0); break;
        case 4: dec_factor = LITDBL(10000.0); break;
        case 5: dec_factor = LITDBL(100000.0); break;
    ...
    if((tmp - TO_SQLITE_DOUBLE(floor(tmp))) == LITDBL(0.5)){
        tmp = itmp / 2;
       if(tmp != TO_SQLITE_DOUBLE(floor(tmp)) ) { // Is itmp odd
         // Reduce Magnitude by 1 to make even
         if(neg) itmp++;
         else itmp--;
       }
    }
	sqlite3_result_double(context, itmp / dec_factor);
}
#endif
...
void sqlite3RegisterBuiltinFunctions(void){
  ...
  */
  static FuncDef aBuiltinFunc[] = {
    ...
    FUNCTION(bround,             1, 0, 0, broundFunc       ),
    FUNCTION(bround,             2, 0, 0, broundFunc       ),
#endif
