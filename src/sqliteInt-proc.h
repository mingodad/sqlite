...
#ifdef SQLITE_STORED_PROCEDURE
typedef struct SP_Variable SP_Variable;
typedef struct SP_Stmt SP_Stmt;
typedef struct SP_SQLStmt SP_SQLStmt;
typedef struct SP_StmtList SP_StmtList;
typedef struct SP_Block SP_Block;
...
#define SF_NOFlattening    0x8000 /* Should skip flattening */
...
#ifdef SQLITE_STORED_PROCEDURE
  SP_Block *pCurrentBlock; 	/* Frame of innermost procedure block being coded */
  SP_Object *pNewObject;		/* Object under construct by a CREATE ... */
  int SP_iLoopExit;        /* Target label of EXIT jump; 0 means we're not inside a loop */
#endif
};
...
struct StrAccum {
  ...
  //u8   useMalloc;       /* For when used as stndalone printf */
};
...
struct SP_Stmt {
  ...
  SP_StmtList *pStmt1;	 /* First sub-statements */
  SP_Stmt *pStmt2;	     /* Second sub-statement (used for IF & CASE)*/
  SP_Block *pBlock;	   /* Block, if this statement is a sub-block */
  SP_SQLStmt *pSql;	   /* SQL statement details, if this is one */
};
...
struct SP_StmtList {
  ...
  struct SP_StmtList_item {
    SP_Stmt *pStmt;         /* The list of statements */
...
struct SP_Block {
  ...
  SP_Variable *aVar;  /* Detail of each local variable */
  ...
  SP_Object *pObj;	   /* Object this body belongs to, if any */
  SP_StmtList *pStList; /* List of statements in this block */
  SP_StmtList *pExList; /* List of exception handlers in this block */
  SP_Block *pParent;  /* Enclosing block, or null if current is outermost */
};
...
#ifdef SQLITE_STORED_PROCEDURE
void sqlite3Proc(Parse *pParse, ExprList *pList);
void sqlite3StartBlock(Parse*, int);
SP_Block *sqlite3EndBlock(Parse*, SP_StmtList*, SP_StmtList*);
...
void sqlite3AddProcVarExpr(Parse *pParse, Expr *pExpr1, int notnull);
int sqlite3LookupVar(Parse*, SP_Block*, Expr*);
SP_Stmt* sqlite3Stmt(Parse*, int, Expr*, SP_StmtList*, SP_Stmt*, SP_Block*);
void sqlite3StmtDelete(sqlite3 *db, SP_Stmt *p);
SP_StmtList *sqlite3StmtListAppend(SP_StmtList *pList, SP_Stmt *pStmt);
void sqlite3StmtListDelete(sqlite3 *db, SP_StmtList *pList);
SP_Stmt *sqlite3SQLStmt(int, Token *, IdList *, ExprList *, Select *, Expr *, int);

void sqlite3FinishProc(sqlite3 *db, Parse*, SP_Block*, Token*);
void sqlite3BeginProc(sqlite3 *db, Parse*, int, Token*);
void sqlite3DropProc(sqlite3 *db, Parse*, Token*);
void sqlite3DeleteObject(sqlite3 *db, SP_Object*);
void sqlite3DropProcPtr(sqlite3 *db, Parse*, SP_Object*);
void sqlite3ExecProc(sqlite3 *db, Parse*, Token*, ExprList*);
void sqlite3ExecBlock(sqlite3 *db, Parse*, SP_Block*);
#endif
